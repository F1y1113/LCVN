...
from scripts.action_utils import generate_bin_tokens, extract_bin_values, DATASET_RANGES, DEFAULT_RANGES, action_to_text, get_dataset_ranges_by_path
from lcvnuni.memory_bank import MemoryBankAnoleForConditionalGeneration
...
class CustomizeSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def _process_single_trajectory(self, raw_item: Dict, output_dir: str, model: Any,
        ...
        # Load initial observation (Goal is generally omitted in Instruction mode)
        ...
        def _get_dataset_ranges(image_path):
            """Determine dataset bounds dynamically based on the path."""
        ...
        # --- 2. Autoregressive Rollout Loop ---
        for step in range(max_steps):
            step_log = {"step": step + 1}
            manager.start_new_step()

            # Build Joint Prompt (Action + Vision)
            joint_prompt = build_instruction_prompt(
            ...
            joint_inputs = self.tokenizer(
                text=[joint_prompt], 
                images=[start_img, current_observation], 
                return_tensors="pt"
            ).to(self.args.device)

            # Prepare generation config (expand max_new_tokens to cover text + image tokens)
            joint_gen_kwargs = action_gen_kwargs.copy()
            joint_gen_kwargs["max_new_tokens"] = self.model.image_token_num + 50 
            joint_gen_kwargs = manager.get_action_kwargs(joint_inputs, joint_gen_kwargs, step)

            with torch.amp.autocast(device_type='cuda', dtype=self.model.dtype):
                joint_outputs = model.generate(**joint_inputs, **joint_gen_kwargs)

            # --- 3. Parse Joint Output ---
            tokens_to_decode = joint_outputs[0].tolist()
            if tokens_to_decode and isinstance(tokens_to_decode[0], list):
                tokens_to_decode = tokens_to_decode[0]

            full_decoded_output = self.tokenizer.decode(tokens_to_decode, skip_special_tokens=False)

            # Split output by <image> token to extract the preceding action text
            image_token = "<image>"
            if image_token in full_decoded_output:
                decoded_action_part = full_decoded_output.split(image_token)[0].strip()
            else:
                decoded_action_part = full_decoded_output.strip()

            is_stop = "stop" in decoded_action_part.lower()

            # Clean and extract action components
            pattern = r'(<d[^>]+>)+(<d[^>]+>)'
            cleaned_action_text = re.sub(pattern, r'\2', decoded_action_part)
            all_decoded.append(cleaned_action_text)

            dx, dy, dyaw = 0.0, 0.0, 0.0
            if not is_stop:
                bin_step = self.action_cfg['bin_step']
                dx = extract_bin_values(cleaned_action_text, "dx", bin_step)
                dy = extract_bin_values(cleaned_action_text, "dy", bin_step)
                dyaw = extract_bin_values(cleaned_action_text, "dyaw", bin_step)
            ...
            # Decode the generated image from the same output sequence
            save_path = os.path.join(traj_output_dir, f"step_{step+1}_observation.png")
            new_obs_img = self._parse_and_save_image(joint_outputs[0], model, self.tokenizer, save_path)
            ...
            if should_stop or step == max_steps - 1 or not new_obs_img:
                if self.is_world_process_zero():
                    if should_stop:
                        print("  Stop action predicted or target reached. Ending trajectory.")
                    elif not new_obs_img:
                        logger.warning("  Failed to generate visualization. Ending trajectory.")
                    else:
                        print("  Max steps reached. Ending trajectory.")
                break

            # Update observation and context memory for the next step
            current_observation = self._sanitize_generated_image(new_obs_img)
            manager.store_step_memory()

        # --- 5. Trajectory Metrics (ATE/RPE) ---
