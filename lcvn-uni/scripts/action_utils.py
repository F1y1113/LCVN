...
DEFAULT_RANGES = {"dxy": [-2.46, 2.46], "dyaw": [-2.82, 2.82]}


# ===================================================================
# 2. Action Calculation Utilities
# ===================================================================
def get_dataset_ranges_by_path(image_path):
    for name, ranges in DATASET_RANGES.items():
        if name != "lcvn" and f"/{name}/" in image_path.replace("\\", "/"):
            return ranges
            
    path_parts = image_path.replace("\\", "/").split('/')
    if len(path_parts) >= 2:
        traj_id = path_parts[-2]
        
        if traj_id.startswith("jackal_"):
            return DATASET_RANGES["recon"]
        elif traj_id.startswith("random_mdps_"):
            return DATASET_RANGES["scand"]
        elif traj_id.startswith(("Jan-", "Feb-", "Mar-", "Apr-", "May-", "Jun-", 
                                 "Jul-", "Aug-", "Sep-", "Oct-", "Nov-", "Dec-")):
            return DATASET_RANGES["sacson"]
        elif traj_id.startswith("no") or traj_id.startswith("sim"):
            return DATASET_RANGES["stanford"]

    return DATASET_RANGES["lcvn"]
