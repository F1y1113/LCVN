...
def unwrap_shortcuts(
    ...
    Example:
    - @latent -> unwrap configurations/latent/base.yaml and configurations/latent/dataset_name.yaml
    - @DiT/social_xl -> unwrap configurations/DiT/social_xl.yaml
    """
    ...
    for arg in argv:
        if arg.startswith("@"):
            shortcut = arg[1:]
            base_path = f"{config_path}/{shortcut}/base.yaml"

            if os.path.exists(base_path):
                new_argv += _yaml_to_cli(base_path)
                dataset_path = f"{config_path}/{shortcut}/{dataset}.yaml"
                if os.path.exists(dataset_path):
                    new_argv += _yaml_to_cli(dataset_path)
            else:
                default_path = f"{config_path}/{shortcut}.yaml"
