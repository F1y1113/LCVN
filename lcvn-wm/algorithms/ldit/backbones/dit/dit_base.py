...
from .ldit_block import LDiTBlock
...
class DiTBase(nn.Module):
    ...
    def __init__(
        ...
        language_layers: Optional[list] = None,
    ):
        """
        Args:
        ...
        self.language_layers = (
            language_layers if language_layers is not None else []
        )
        ...
        for i in range(depth):
            if self.use_language and i in self.language_layers:
                blocks.append(
                    LDiTBlock(
    ...
    def forward(
        ...
        Args:
            ...
            i_clip: Optional instruction features of shape (B, L, C).
        Returns:
        ...
        for i, (block, temporal_block) in enumerate(
            ...
            if hasattr(block, 'use_language') and block.use_language:
                execute_in_parallel(
                    lambda x, c, batch_size: self.checkpoint(block, x, c, i_clip)
                )
            else:
