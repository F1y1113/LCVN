...
# In pure inference scenarios, importing video metrics can trigger heavy third-party dependencies; make them optional
try:
    ...
    VideoMetric = None
    SharedVideoMetricModelRegistry = None
...
class DFoTVideo(BasePytorchAlgo):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.generator = None
        # Ensure max_tokens is available during early _build_model invocation
        try:
            self._max_tokens = (self.cfg.max_frames - 1) * self.temporal_downsampling_factor + 1
    ...
    def _build_model(self):
        ...
        self.diffusion_model = torch.compile(
            diffusion_cls(
                ...
                max_tokens=(self.cfg.max_frames - 1) * self.temporal_downsampling_factor + 1,
    ...
    def _load_vae(self) -> None:
        """
        Load the pretrained VAE model.
        Supports both HF/Diffusers format and local .ckpt files.
        """
        path = self.cfg.vae.pretrained_path
        print(f"[DFoT] Loading VAE from: {path}")

        if str(path).endswith(".ckpt"):
            # Case A: local .ckpt file
            print(f"[DFoT] Detected local checkpoint, using torch.load...")
            checkpoint = torch.load(path, map_location="cpu", weights_only=False)

            # Initialize model structure
            if "cfg" in checkpoint:
                self.vae = ImageVAE(checkpoint["cfg"])
            else:
                print("[DFoT] Warning: No 'cfg' in VAE checkpoint, using default 256 config.")
                self.vae = ImageVAE(OmegaConf.create({"z_channels": 4, "resolution": 256}))

            # Load weights
            state_dict = checkpoint["state_dict"] if "state_dict" in checkpoint else checkpoint
            # Filter out loss keys
            state_dict = {k: v for k, v in state_dict.items() if not k.startswith("loss")}

            # Load (strict=False)
            msg = self.vae.load_state_dict(state_dict, strict=False)
            print(f"[DFoT] VAE weights loaded. Msg: {msg}")

            self.vae = self.vae.to(self.device)

        else:
            # Case B: Diffusers/HF loading
            vae_cls = VideoVAE if self.is_latent_video_vae else ImageVAE
            self.vae = vae_cls.from_pretrained(
                path=path,
                torch_dtype=(
    ...
    def _get_i_clip(self, rest) -> Optional[Tensor]:
        """
        Extract instruction features from batch rest args.
        Base class returns None; subclasses override to encode instructions.
        """
        return None
    ...
    def on_after_batch_transfer(
        ...
        Args:
            batch (Dict): The batch of data. Contains "videos" or "latents", (optional) "conditions", "masks", and "instruction".
            dataloader_idx (int): The index of the dataloader.
        Returns:
            xs (Tensor): Tokens to be processed by the model.
            conditions (Optional[Tensor]): External conditions for the tokens.
            masks (Tensor): Masks for the tokens.
            gt_videos (Optional[Tensor]): Optional ground truth videos.
            instructions (Optional[List[str]]): Instruction strings from the dataset.
        """
        ...
        # 4. Prepare instruction strings (if available from dataset)
        instructions = batch.get("instruction", None)

        return xs, conditions, masks, gt_videos, instructions
    ...
    def training_step(self, batch, batch_idx, namespace="training") -> STEP_OUTPUT:
        """Training step"""
        xs, conditions, masks, *rest = batch
        i_clip = None  # subclasses may override to encode instructions
    ...
    def on_train_epoch_end(self) -> None:
        ...
        Manually log epoch-level loss for checkpoint monitoring.
        """
        if 'training/loss' in self.trainer.callback_metrics:
            epoch_loss = self.trainer.callback_metrics['training/loss']
            self.log('train_loss_epoch', epoch_loss, sync_dist=True)
            print(f"✅ Logged train_loss_epoch: {epoch_loss:.4f}")
    ...
    def _sample_all_videos(
        ...
        for task in self.tasks:
            ...
            all_videos[task] = sample_fn(
                xs, conditions=conditions, i_clip=i_clip
            )
        ...
        if self.is_latent_diffusion:
            ...
            # Check raw latent signal before VAE decode
            print(f"\n[DEBUG] Checking model output latent signal (before VAE)...")
            target_key = "prediction" if "prediction" in all_videos else None
            if target_key is None and "interpolation" in all_videos:
                target_key = "interpolation"

            if target_key:
                check_target = all_videos[target_key]

                l_mean = check_target.float().mean().item()
                l_std = check_target.float().std().item()
                l_min = check_target.float().min().item()
                l_max = check_target.float().max().item()

                print(f"[DEBUG] Task: {target_key}")
                print(f"[DEBUG] Shape: {check_target.shape}")
                print(f"[DEBUG] Stats: Mean={l_mean:.4f}, Std={l_std:.4f}, Min={l_min:.4f}, Max={l_max:.4f}")

                if l_std < 0.05:
                    print("[DEBUG] Signal dead (flatline) — model outputs solid color. Retrain required.")
                elif l_std > 0.5:
                    print("[DEBUG] Signal active — model has output. If frames are black, check VAE decode.")
                else:
                    print("[DEBUG] Signal weak — possible missing scaling factor.")
            else:
                print("[DEBUG] No generated video latent found.")
    ...
    def _interpolate_videos(
        ...
        for current_plan in plan:
            ...
            # Expand i_clip for the batched interpolation chunks
            n_chunks = len(current_plan)
            _inst_feat_interp = None
            if i_clip is not None and n_chunks > 0:
                _inst_feat_interp = i_clip.repeat(n_chunks, 1, 1)
            ...
            inst_feat_splits = (
                _inst_feat_interp.split(max_batch_size, 0)
                if _inst_feat_interp is not None
                else [None] * max(1, current_context.shape[0] // max_batch_size)
            )
            for (
                current_context_chunk,
                current_context_mask_chunk,
                current_conditions_chunk,
                current_inst_feat_chunk,
            ) in zip(
                ...
                xs_pred_chunk, _ = self._sample_sequence(
                    ...
                    history_guidance=history_guidance,
                    pbar=pbar,
                    i_clip=current_inst_feat_chunk,
                )
    ...
    def _sample_sequence(
        ...
        for m in range(scheduling_matrix.shape[0] - 1):
            ...
            with history_guidance(context_mask) as history_guidance_manager:
                ...
                # expand i_clip for NFE if needed
                _inst_feat = i_clip
                if _inst_feat is not None and nfe > 1:
                    _inst_feat = repeat(
                        _inst_feat, "b ... -> (b nfe) ...", nfe=nfe
                    ).clone()
