    ...
    if cfg.wandb.mode != "disabled":
        ...
        while not (project_root / ".git").exists():
            ...
            if project_root == Path("/"):
                break
        wandb_base = Path(os.getenv("WANDB_DIR", str(Path.cwd() / "wandb")))

        # If default path is not writable, redirect to wandb folder in current directory
        if not os.access(wandb_base.parent, os.W_OK):
            wandb_base = Path.cwd() / "wandb"
            wandb_base.mkdir(parents=True, exist_ok=True)

        os.environ["WANDB_DIR"] = str(wandb_base)
        os.environ["WANDB_CACHE_DIR"] = os.getenv("WANDB_CACHE_DIR", str(wandb_base / ".cache"))
        os.environ["WANDB_DATA_DIR"] = os.getenv("WANDB_DATA_DIR", str(wandb_base / ".data"))
...
def run(cfg: DictConfig):
    ...
    if not cfg.wandb.get("entity", None) and os.environ.get("WANDB_MODE", "online") not in ("offline", "disabled"):
        raise ValueError(
