...
class BaseLightningExperiment(BaseExperiment):
    ...
    def training(self) -> None:
        ...
        # --- Auto-copy checkpoint callback ---
        import shutil, os
        from lightning.pytorch.callbacks import Callback

        class AutoCopyCheckpointCallback(Callback):
            def __init__(self, algo_name):
                self.algo_name = algo_name

            def on_train_end(self, trainer, pl_module):
                ckpt_cb = trainer.checkpoint_callback
                best_ckpt = getattr(ckpt_cb, "best_model_path", None) or getattr(ckpt_cb, "last_model_path", None)
                if ckpt_cb and best_ckpt:
                    if not os.path.exists(best_ckpt):
                        return

                    filename = None
                    if "vae" in self.algo_name:
                        filename = "vae.ckpt"
                    elif "ldit" in self.algo_name:
                        filename = "social_dit_xl.ckpt"
                    else:
                        print(f"[AutoCopy] Unknown algo '{self.algo_name}', skipping copy.")

                    if filename:
                        # Save to ../outputs relative to the execution directory (lcvn-wm)
                        target = os.path.abspath(os.path.join(hydra.utils.get_original_cwd(), "..", "outputs", filename))
                        os.makedirs(os.path.dirname(target), exist_ok=True)
                        print(f"\n[AutoCopy] Copying final checkpoint to {target}")
                        try:
                            if os.path.exists(target):
                                os.remove(target)
                            shutil.copy(best_ckpt, target)
                            print(f"[AutoCopy] Successfully updated {filename}")
                        except Exception as e:
                            print(f"[AutoCopy] Failed to copy checkpoint: {e}")
        algo_name = self.root_cfg.algorithm.get("_name", "")
        callbacks.append(AutoCopyCheckpointCallback(algo_name))
        # -------------------------------------
        ...
        trainer = pl.Trainer(
            ...
            detect_anomaly=self.cfg.training.get('detect_anomaly', False),
