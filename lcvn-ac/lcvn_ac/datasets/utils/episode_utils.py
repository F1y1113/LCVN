...
def get_state_info_dict(
    episode: Dict[str, np.ndarray], 
    for_wm: bool, 
    proprio_state: Optional[DictConfig] = None
) -> Dict[str, Dict[str, torch.Tensor]]:
    """
    Create a dictionary with raw state observations for environment resets.

    This function is robust to datasets that do not provide 'robot_obs'. In that case,
    it creates a zero tensor with the correct time length and feature dimension.

    Args:
        episode: Sequence dictionary.
        for_wm: Whether dataset is for world model training.
        proprio_state: Optional proprioception config with 'n_state_obs'.

    Returns:
        Info dict of full robot and scene state (for env resets).
    """
    info: Dict[str, Dict[str, torch.Tensor]] = {"state_info": {}}

    # Determine time length T
    T = 0
    if "rel_actions" in episode and isinstance(episode["rel_actions"], np.ndarray):
        T = int(episode["rel_actions"].shape[0])
    elif "vae_latent" in episode and isinstance(episode["vae_latent"], np.ndarray):
        T = int(episode["vae_latent"].shape[0])

    # Number of state features
    n_state_obs = 0
    if proprio_state is not None and hasattr(proprio_state, "n_state_obs"):
        n_state_obs = int(proprio_state.n_state_obs)

    # Robot observations (fallback to zeros if missing)
    if "robot_obs" in episode:
        info["state_info"]["robot_obs"] = torch.from_numpy(episode["robot_obs"])  # type: ignore
    else:
        info["state_info"]["robot_obs"] = torch.zeros((T, n_state_obs), dtype=torch.float32)

    # Optional pre_robot_obs for WM
    if for_wm:
        if "pre_robot_obs" in episode:
            info["state_info"]["pre_robot_obs"] = torch.from_numpy(episode["pre_robot_obs"])  # type: ignore
        else:
            info["state_info"]["pre_robot_obs"] = torch.zeros((1, n_state_obs), dtype=torch.float32)
...
def load_dataset_statistics(train_dataset_dir, val_dataset_dir, transforms):
    ...
    for dataset_type in ["train", "val"]:
        try:
            ...
            statistics = OmegaConf.create(OmegaConf.to_yaml(statistics).replace("calvin_agent.", "lcvn_ac."))
