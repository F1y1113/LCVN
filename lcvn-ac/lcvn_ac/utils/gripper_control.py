...
import numpy as np
from lcvn_ac.utils.pytorch3d_transforms import (
...
def world_to_tcp_frame(action, robot_obs):
    with autocast(dtype=torch.float32):
        ...
        # upscaling again
        orn_tcp_rel *= 100
        action_tcp = torch.cat([pos_tcp_rel.view(s, b, -1), orn_tcp_rel.view(s, b, -1), action[..., -1:]], dim=-1)
...
def tcp_to_world_frame(action, robot_obs):
    with autocast(dtype=torch.float32):
        ...
        orn_w_rel = orn_w_new - robot_obs[..., 3:6].reshape(-1, 3)  # .view(-1, 3)
        orn_w_rel = torch.where(orn_w_rel < -np.pi, orn_w_rel + 2 * np.pi, orn_w_rel)
        orn_w_rel = torch.where(orn_w_rel > np.pi, orn_w_rel - 2 * np.pi, orn_w_rel)
        # upscaling again
        orn_w_rel *= 100
        action_w = torch.cat([pos_w_rel.view(s, b, -1), orn_w_rel.view(s, b, -1), action[..., -1:]], dim=-1)
